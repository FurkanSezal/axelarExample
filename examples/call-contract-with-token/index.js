...
async function test(chains, wallet, options) {
    ...
    async function logAccountBalances() {
        for (const account of accounts) {
            console.log(
                `${destination.contract.address} has ${(await destination.usdc.balanceOf(destination.contract.address)) / 1e6} aUSDC`,
            );
        }
    }
    ...
    const balance = BigInt(await destination.usdc.balanceOf(destination.contract.address));

    const approveTx = await source.usdc.approve(source.contract.address, amount);
    await approveTx.wait();

    const message = 'Buy a coffee from me!';
    const payload = ethers.utils.defaultAbiCoder.encode(['string'], [message]);

    const sendTx = await source.contract.sendTokenWithMessage(
        destination.name,
    ...
    while (BigInt(await destination.usdc.balanceOf(destination.contract.address)) === balance) {
        await sleep(2000);
    }

    console.log('--- After ---');
    await logAccountBalances();
    const [text, value] = await destination.contract.seeMessage(wallet.address);

    const decodedText = ethers.utils.defaultAbiCoder.decode(['string'], text);

    console.log(`You send ${(value / 1e6).toString()} aUsdc with message: ${decodedText}`);
}
